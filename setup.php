<?php
  ...
  if(!tableExists($pdo, 'cachebank_webhook_logs')){
    $pdo->exec(file_get_contents('/opt/mk-auth/admin/addons/cachebank/db/cachebank_webhook_logs.sql'));
  }
  if(!tableExists($pdo, 'cachebank_invoices')){
    $pdo->exec(file_get_contents('/opt/mk-auth/admin/addons/cachebank/db/mkradius.cachebank_invoices.sql'));
  }
  if(!tableExists($pdo, 'cachebank_config')){
    $pdo->exec(file_get_contents('/opt/mk-auth/admin/addons/cachebank/db/mkradius.cachebank_config.sql'));
  }
...
RewriteRule ^boleto.hhvm(.*)$ /cachebank/cachebank-view-boleto.php$1 [R=301,NC]';
    ...
    if(!strpos(file_get_contents($htaccessFileBoleto), $htaccesstoAdd1) !== false) {
     ...
     /* $currentHtaccess = file_get_contents($htaccessFileBoleto);
      $currentHtaccess=str_replace($htaccesstoAdd1, "", $currentHtaccess);
      file_put_contents($htaccessFileBoleto, $currentHtaccess);*/
    }
...
/*
      $currentHtaccess = file_get_contents($htaccessFileBoleto);
      $currentHtaccess=str_replace($htaccesstoAdd2, "", $currentHtaccess);
      file_put_contents($htaccessFileBoleto, $currentHtaccess);*/
    }
